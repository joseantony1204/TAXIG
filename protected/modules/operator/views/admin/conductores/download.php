...
$objPHPExcel->setActiveSheetIndex($index)
								 ...
								 ->setCellValue('B3', 'RELACION DE TAXISTAS')
...
foreach($registros as $data){
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('H'.$fxls, $data['VEHI_NUMEROMOVIL']);
 $objPHPExcel->getActiveSheet()->setCellValue('I'.$fxls, $Empresas->EMPR_NOMBRE);
 ...
 $item++; 
  ...
  /*CREANDO HOJAS PARA CADA PROGRAMA INCLUYENDO SUS RESPECTIVAS CATEDRAS ASIGNADAS*/
   $objPHPExcel->setActiveSheetIndex($index);
   $nombre = substr('REPORTE_TAXISTAS',0,20);
   $objPHPExcel->getActiveSheet()->setTitle("$nombre");
   $index++;
   
   $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
  $nombreArch = strtoupper("TAXISTAS");
