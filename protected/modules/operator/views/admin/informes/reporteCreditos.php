...
$objPHPExcel->getActiveSheet()->getStyle('B9:N9')->applyFromArray($styleArrayB);

$objPHPExcel->getActiveSheet()->getStyle('H8:N8')->applyFromArray($styleArrayB);
...
$deudaPrestamo = ($data["PRESTAMOS"])-$data["ABONOTOTAL"];
 ...
 $item++; 
  ...
  $objPHPExcel->getActiveSheet()->setAutoFilter('B9:N'.$fxls); 
  ...
  /* totales */
   ...
   $objPHPExcel->getActiveSheet()->getStyle('H'.$fxls.':N'.$fxls)->getNumberFormat()->setFormatCode('#,##0'); 
   $objPHPExcel->getActiveSheet()->getStyle('G'.$fxls.':N'.$fxls)->applyFromArray($styleArrayB);

 
  /*CREANDO HOJAS PARA CADA PROGRAMA INCLUYENDO SUS RESPECTIVAS CATEDRAS ASIGNADAS*/
   $objPHPExcel->setActiveSheetIndex($index);
   $nombre = substr('EXTRACTO_PRESTAMOS',0,20);
