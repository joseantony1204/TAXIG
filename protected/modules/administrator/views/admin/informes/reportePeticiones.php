...
$objPHPExcel->setActiveSheetIndex($index)
								 ...
								 ->setCellValue('H8', 'CONTENIDO DE MANIFESTACION')
								 ->setCellValue('I8', 'SUGERENCIAS')
								 ->setCellValue('J8', 'FECHA DE INGRESO') 
								 ->setCellValue('K8', 'RESPUESTA')
								 ->setCellValue('L8', 'FECHA DE RESPUESTA');
					
$fxls=10;
$item = 1;

$objPHPExcel->getActiveSheet()->getStyle('B1:B6')->applyFromArray($styleArray);
$objPHPExcel->getActiveSheet()->getStyle('B8:L8')->applyFromArray($styleArrayB);
...
foreach($peticiones as $data){
 ...
 $objPHPExcel->getActiveSheet()->getStyle('B'.$fxls.':'.'L'.$fxls)->applyFromArray($styleArrayBInt);
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('H'.$fxls, $data["PEQR_CONTENIDO"]);
 $objPHPExcel->getActiveSheet()->setCellValue('I'.$fxls, $data["PEQR_SUGERENCIAS"]);
 $objPHPExcel->getActiveSheet()->setCellValue('J'.$fxls, $data["PEQR_FECHAREGISTRO"]);
 $objPHPExcel->getActiveSheet()->setCellValue('K'.$fxls, $Respuestaspqr->RPQR_DESCRIPCION);
 $objPHPExcel->getActiveSheet()->setCellValue('L'.$fxls, $Respuestaspqr->RPQR_FECHAINGRESO);
