...
$objPHPExcel->setActiveSheetIndex($index)
								 ...
								 ->setCellValue('G8', 'FECHA DE SERVICIO')
								 ->setCellValue('H8', 'FECHA DE PAGO')
								 ->setCellValue('I8', 'CONCEPTO')
								 ->setCellValue('J8', 'VALOR PAGADO')
...
foreach($pagos as $data){
 ...
 $diaS = date("d",strtotime($data['SERVI_FECHAINGRESO']));
 $mesS = nombreMes(date("m",strtotime($data['SERVI_FECHAINGRESO'])));
 $anioS = date("Y",strtotime($data['SERVI_FECHAINGRESO']));  
 $fechaS = $diaS." DE ".$mesS." DE ".$anioS;
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('G'.$fxls, strtoupper($fechaS)); 
 $objPHPExcel->getActiveSheet()->setCellValue('H'.$fxls, strtoupper($fecha)); 
 $objPHPExcel->getActiveSheet()->setCellValue('I'.$fxls, $data['CONC_NOMBRE']);
 $objPHPExcel->getActiveSheet()->setCellValue('J'.$fxls, $data['PACO_VALOR']);
 $objPHPExcel->getActiveSheet()->setCellValue('K'.$fxls, $Personasnaturales->PENA_NOMBRES); 
 ...
 $item++;
  ...
  /* totales */
   $fxls = $fxls + 1;
   $objPHPExcel->getActiveSheet()->setCellValue('I'.$fxls, "TOTAL");
   $objPHPExcel->getActiveSheet()->setCellValue('J'.$fxls, $totalPago);
   $objPHPExcel->getActiveSheet()->getStyle('J'.$fxls)->getNumberFormat()->setFormatCode('#,##0');
   $objPHPExcel->getActiveSheet()->getStyle('I'.$fxls.':K'.$fxls)->applyFromArray($styleArrayB);
