...
$file=("Factura");
$modo="w";
$arc = new Archivo($file.".dat",$modo);

 $NombreDocumento = $NombreDocumento.$Pagos->PAGO_ID;
 $Usuarios = Usuarios::model()->findByPk($Pagos->USUA_ID);
...
$encabezado=$arc->bajar(1).$arc->centro("TAXI GUAJIRA S.A.S",32).$arc->enter().
$arc->centro("NIT 900.381.888 - 1 ",32).$arc->enter().
$arc->centro("TEL 7283000 - 3002601151",32).$arc->enter().
$arc->centro("CALLE 27B #19-97, RCHA, GUAJIRA",32).$arc->enter().
...
$arc->enter().$arc->izquierda("F. PROCESO: ".date("Y-m-d H:i:s"),32);
...
$arc->escribir($arc->bajar(3).'RECIBI:'.$Personasnaturales->PENA_NOMBRES.' '.$Personasnaturales->PENA_APELLIDOS);

$arc->escribir($arc->enter().$arc->enter().'***** COMPROBANTE DE PAGO *****').$arc->bajar(6);
$arc->cerrar();
?>